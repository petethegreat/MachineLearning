...
ESL recommends sqrt of number of variables for classification. There are 53 predictors, so around mtry=7-8. Will try a coarse grid to start, then refine it
...
endtime - starttime
```
Will aim for 95% variance, so will keep 25 components. Need to specify components, because otherwise the number of predictors might change for each cross validation fold.
...
Took a long time for only 4 mtry values and a reduced number of predictors.
rangerPCA stuff
